...
You could use `backgroundColor` prop.

Default colours are:

```
backgroundColor="bulma.100"
```
...
Toast could appear without warning, and don't require user interaction.

```jsx react-live
() => {
  const [Toast, display, hide] = useToast({
...
Toast should be placed at the bottom of a UI, in front of app content.
Avoid placing a Toast in front of frequently used touch targets or navigation.

By default it has inline position.

It has same z-index as Dialog.
