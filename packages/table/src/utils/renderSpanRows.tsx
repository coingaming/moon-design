...
const renderSpanRows = ({
  ...
  return rows?.map((row: any, index: number) => {
    ...
    const isRowSpanned = rowSpanHeaders?.some((rowSpanHeader: any) => row.cells.some((cell: any) => rowSpanHeader.id === cell.column?.id && rowSpanHeader.value === cell.value));
    ...
    return (
      <BodyTR
        ...
        {row.cells.map((cell: any) => {
          const rowSpanHeader = rowSpanHeaders?.find(rowSpanHeader => rowSpanHeader?.id === cell.column?.id);
