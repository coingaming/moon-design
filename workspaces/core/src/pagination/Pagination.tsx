...
const Pagination: React.FC<PaginationProps> = ({
  ...
  const renderPreviousLabel = () => {
    if (typeof previousButtonLabel === 'string') {
      return (
        <div>
          <Button className="hidden sm:flex">{previousButtonLabel}</Button>
  ...
  const renderNextLabel = () => {
    if (typeof nextButtonLabel === 'string') {
      return (
        <div>
          <Button className="hidden sm:flex">{nextButtonLabel}</Button>
          <div className="sm:hidden h-8 w-8 flex items-center justify-center border border-trunks rounded-lg">
  ...
  return (
    <div className="flex flex-wrap items-center justify-between">
      {!!pageSizeSection && <div className="m-2.5"> {pageSizeSection} </div>}
      <div className="flex flex-column flex-wrap items-center justify-center md:flex-row">
        <ReactPaginate
          ...
          previousClassName={`my-2.5 text-center me-9 text-trunks ${
            pageNumber === 1 && 'cursor-default'
          }`}
          nextClassName={`my-2.5 text-center ms-9 text-trunks ${
            pageNumber === pageCount && 'cursor-default'
          }`}
      ...
      {!!goToPageSection && <div className="m-2.5"> {goToPageSection} </div>}
    </div>
  );
};
