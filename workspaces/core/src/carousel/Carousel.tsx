...
const Indicators: React.FC<SubcomponentProps> = ({ className }) => {
  const { itemsCount, selectedIndex, firstVisibleIndex = 0, lastVisibleIndex = 0 } = useCarouselContext('Carousel.RightArrow');
  const activeIndex = !selectedIndex || selectedIndex < firstVisibleIndex || selectedIndex > lastVisibleIndex ? firstVisibleIndex : selectedIndex;
  const items = Array.from({ length: itemsCount ?? 0 }, (index) => index);
  return (
    <div className={mergeClassnames('flex absolute bottom-8 left-1/2 -translate-x-1/2', className)}>
      {items?.map((_, index) => (
        <div className={mergeClassnames('w-2 h-2 mx-1 rounded-full bg-beerus', activeIndex === index && 'bg-piccolo')} key={index} />
      ))}
    </div>
  );
};
