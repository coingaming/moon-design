...
const RTLModeSwitch: React.FC<IRTLModeSwitch> = ({ isEnabled, toggle }) => (
  ...
  >
    ...
    <span
      className={`bg-gohan text-bulma pointer-events-none relative inline-block h-5 w-5 rounded-full shadow transform ring-0 transition ease-in-out duration-200`}
    >
      <span
        className={`${
          isEnabled
